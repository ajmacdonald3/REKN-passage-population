...
enchist_pxda <- enchist_csv %>% 
  add_row(FlagID = paste0("PXDA", 1:1000)) %>%   
  replace(., is.na(.), "0")
...
source("analysis-code/JAGScode_phicpt.R")
...
inits <- function(){list(mean.phi = runif(1, 0, 1),
                         psight = runif(dim(enchist)[2], 0, 1),
                         z = z.init,
                         alpha = runif(1, -1, 1),
                         delta = runif(15, -1, 1),
...
parameters <- c("psi", "mean.phi", "psight", "b", "delta", "pflag", "meanpflag",
...
# Call JAGS from R
ALLCAMPS2018_phicpt <- jags(jags_data, inits, parameters,
                            "RDO-model.jags",
...
# view results
print(ALLCAMPS2018_phicpt, digits = 3)

# traceplots
pdf(file = paste0("./analysis-output/ALLCAMPS2018_RDO_phicpt_traceplots_", Sys.Date(), ".pdf"))
par(mfrow = c(3, 1))
traceplot(ALLCAMPS2018_phicpt)
dev.off()

# export results as CSV file
write.csv(ALLCAMPS2018_phicpt$summary,
          file = paste0("./analysis-output/ALLCAMPS2018_RDO_phicpt_summary_", Sys.Date(), ".csv"))

# export more results for stopover duration
write.csv(ALLCAMPS2018_phicpt$sims.list,
          file = paste0("./analysis-output/ALLCAMPS2018_RDO_phicpt_sims.list_", Sys.Date(), ".csv"))
